...
export async function POST(request: Request, context: { params: Promise<{ userId: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ message: "Passwort-Reset wurde versendet." });
  } catch (error) {
    console.error("Passwort-Reset konnte nicht versendet werden.", error);
    const isDev = process.env.NODE_ENV !== "production";
    const message =
      isDev && error instanceof Error
        ? `Passwort-Reset konnte nicht versendet werden: ${error.message}`
